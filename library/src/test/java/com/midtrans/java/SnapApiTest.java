...
public class SnapApiTest {
    ...
    public void badRequestBodyOnSnapTrans() {
        try {
            ...
            assert result.getJSONArray("error_messages").get(0).toString().equals("transaction_details.gross_amount is required");
            assert result.getJSONArray("error_messages").get(1).toString().equals("transaction_details.gross_amount is not a number");
